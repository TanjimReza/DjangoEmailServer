...
def get_household_url_profile(email_body):
    print("GET HOUSEHOLD URL FUNCTION")
    ...
    print(f"RETURNING: {profile}, {get_code_url}")
...
def check_emails():
    ...
    with imaplib.IMAP4_SSL(settings.EMAIL_SERVER) as mail:
        mail.login(settings.EMAIL_ACCOUNT, settings.EMAIL_PASSWORD)
        mail.select("inbox")
        try: 
            typ, messages = mail.search(None, settings.EMAIL_FILTER)
            ...
            for idx, message_id in enumerate(message_ids):
                ...
                # call save_email_to_db
                print("Email CATEGORY", email_dict["tag"])
                print("Email HOUSEHOLD LINK", email_dict["household_link"])
                if email_dict["tag"] != "OTHER":
                    save_email_to_db(email_dict)
        ...
        # logging.info(f"Email Body: {email_dict['email_body'].replace('\n', '.')}")
